...
return (
    <>
    <div className="flex flex-col md:flex-row flex-no-wrap bg-teal-300">
    <div id="Certificate Image Selector" className="bg-blue-200 p-2  max-w-xl">
    <h1>My Certificates</h1>
      <ul>
       {arrayOfCerts.map((cert) => (
       <button className="bg-blue-600 text-white rounded text-sm p-1" onClick={() => setSRC(cert.src)}><li>{cert.name}</li></button>
       ))
      }
      </ul>
    </div>
